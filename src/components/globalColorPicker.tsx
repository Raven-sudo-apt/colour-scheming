import { useEffect, useRef, useState, type KeyboardEvent as RKeyEvent, type PointerEvent as RPointerEvent } from 'react'
...
type Props = {
    ...
    label?: string
    swatches?: string[] // optional quick-pick colors
}

const W = 240 // popover width
const H = 330 // approximate popover height, used for positioning
...
export default function ColorPicker({ value, onChange, label = 'color', swatches = [] }: Props) {
    const [open, setOpen] = useState(false)
    const [pos, setPos] = useState({ top: 0, left: 0 })
    const [hsv, setHsv] = useState(() => hexToHsv(value))
    const [prev, setPrev] = useState(value)
    const [draft, setDraft] = useState<string | null>(null) // hex text while typing
    ...
    const toggle = () => {
        if (!open) {
            ...
            const top = Math.max(8, Math.min(r.top - 16, window.innerHeight - H - 8))
            setPos({ top, left })
        }
        setOpen((o) => !o)
    }
